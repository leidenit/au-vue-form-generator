	...
	export default {
		...
		methods: {
			...
			fieldDisabled(field) {
                ...
                var that = this, depend = false;
                if (isArray(field.disabled)) {
                    field.disabled.forEach(function (element) {
                        if (typeof element === 'object')
                            if (!that[element.condition](element.target, element.value))
                                depend = true;
                    });
                }
                return depend;
			...
			fieldVisible(field) {
				...
				var that = this, depend = true;
				if (isArray(field.visible)) {
                    field.visible.forEach(function (element) {
                        if (typeof element === 'object')
                            if (!that[element.condition](element.target, element.value))
                                depend = false;
                    });
            ...
            // Any off
            anyof(target, value) {
                if(isArray(this.model[target]))
                    this.model[target].forEach(function (element) {
                        if(value.indexOf(element) > -1)
                            return true;
                    });

                if(isArray(value) && value.indexOf(this.model[target]) > -1 || this.model[target] == value)
                    return true;
            ...
            anyex(target, value) {
                if(isArray(this.model[target]))
                    if(this.model[target].length >= 1)
                        return true;

                return !(this.model[target] == "" || this.model[target] == value);
            },
